# Table of contents

* [Get Started](README.md)

## The Basics

* [Customers](the-basics/customers/README.md)
  * [Reference](the-basics/customers/reference/README.md)
    * [API](https://docs.developers.zaius.com/api/rest-api/customers)
    * [Web SDK](https://docs.developers.zaius.com/web-sdk/customers/attributes)
* [Events](the-basics/events/README.md)
  * [Reference](the-basics/events/reference/README.md)
    * [API](https://docs.developers.zaius.com/api/rest-api/events)
    * [Web SDK](https://docs.developers.zaius.com/web-sdk/events/custom-events)
    * [iOS SDK](https://docs.developers.zaius.com/ios-sdk/sdk-methods/customers)
    * [Android SDK](https://docs.developers.zaius.com/android-sdk/sdk-methods/events)
* [Objects & Fields](the-basics/objects-and-fields/README.md)
  * [Creating Objects & Fields](the-basics/objects-and-fields/creating-objects-fields/README.md)
    * [API](https://docs.developers.zaius.com/api/rest-api/advanced/schema)
  * [Updating Objects & Fields](the-basics/objects-and-fields/updating-objects/README.md)
    * [API](https://docs.developers.zaius.com/api/rest-api/objects)

## Use Cases

* [Marketing & Channels](use-cases/marketing/README.md)
  * [Consent](use-cases/marketing/consent-1/README.md)
    * [API](https://docs.developers.zaius.com/api/rest-api/consent)
    * [Web SDK](https://docs.developers.zaius.com/web-sdk/customers/consent)
  * [Lists](use-cases/marketing/lists/README.md)
    * [API](https://docs.developers.zaius.com/api/rest-api/lists)
    * [Web SDK](https://docs.developers.zaius.com/web-sdk/customers/lists)
* [eCommerce](use-cases/ecommerce/README.md)
  * [Orders](use-cases/ecommerce/orders/README.md)
    * [API](https://docs.developers.zaius.com/api/rest-api/orders)
    * [CSV](https://docs.developers.zaius.com/client-onboarding)
  * [Products](use-cases/ecommerce/products/README.md)
    * [API](https://docs.developers.zaius.com/api/rest-api/products)
    * [CSV](https://docs.developers.zaius.com/client-onboarding)
* [Loyalty & Rewards](use-cases/loyalty-and-rewards.md)

